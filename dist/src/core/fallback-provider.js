...
class FallbackProvider {
    ...
    _calculatePriority(text) {
        if (!text)
            return 1;
        if (text.length < 100)
            return 2; // Higher priority for shorter texts
        if (text.length > 800)
            return 0; // Lower priority for very long texts
        return 1;
    }
    async translate(text, sourceLang, targetLang, options) {
        ...
        while (currentAttempt < maxAttempts) {
            ...
            try {
                ...
                const result = await rate_limiter_1.default.enqueue(providerName.toLowerCase(), () => retry_helper_1.default.withRetry(
    ...
    async analyze(prompt, options = {}) {
        ...
        while (currentAttempt < maxAttempts) {
            ...
            try {
                ...
                const providerStartTime = Date.now();
                const result = await rate_limiter_1.default.enqueue(providerName.toLowerCase(), () => retry_helper_1.default.withRetry(() => currentProvider.analyze(prompt, options), {
                    maxRetries: 0,
                    context: `Fallback:${providerName}`,
                    logContext: {
                        providerIndex: currentProviderIndex,
    ...
    _checkAndReRankProviders() {
        ...
        const providerRanks = this.providers.map((provider, index) => {
            ...
            if (total > 0) {
                ...
                const responseTimePenalty = stats.avgResponseTime > 0
...
exports.default = FallbackProvider;
