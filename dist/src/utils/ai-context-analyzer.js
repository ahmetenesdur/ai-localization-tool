...
exports.AIContextAnalyzer = void 0;
const provider_factory_1 = __importDefault(require("../core/provider-factory"));
...
class AIContextAnalyzer {
    ...
    async analyzeContext(text, apiProvider = "openai") {
        ...
        if (this.cache.has(cacheKey)) {
            ...
            if (cached) {
                const { explanation, ...contextData } = cached;
                return contextData;
            }
            return null;
        }
        ...
        try {
            ...
            const provider = provider_factory_1.default.getProvider(apiProvider, true, null);
            if (!provider) {
                throw new Error("AI provider not available for context analysis");
            }
            const analysisPrompt = text.length < 500
                ? this._createSimplifiedAnalysisPrompt(text)
                : this._createAnalysisPrompt(text);
            const result = await rate_limiter_1.default.enqueue(apiProvider.toLowerCase(), () => provider.analyze(analysisPrompt, this.config.analysisOptions));
...
exports.AIContextAnalyzer = AIContextAnalyzer;
exports.default = AIContextAnalyzer;
//# sourceMappingURL=ai-context-analyzer.js.map