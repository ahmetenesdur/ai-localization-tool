...
class InputValidator {
    ...
    static validateProvider(provider, paramName = "provider") {
        ...
        if (!this.VALID_PROVIDERS.includes(normalized)) {
            throw new Error(`Invalid ${paramName}: '${provider}'. Valid providers: ${this.VALID_PROVIDERS.join(", ")}`);
        }
        return normalized;
    }
    /**
    ...
    static createSafeFilePath(baseDir, filename) {
        ...
        const fullPath = path_1.default.join(baseDir, safeFilename);
        ...
        if (!resolved.startsWith(resolvedBase + path_1.default.sep) && resolved !== resolvedBase) {
    ...
    static validateConfig(config) {
        ...
        if (validated.localesDir) {
            validated.localesDir = this.validateDirectoryPath(validated.localesDir, "locales directory");
        }
 ...
 */
InputValidator.LANGUAGE_CODE_PATTERN = /^[a-z]{2}(-[a-z]{2})?$/;
/**
 * Valid provider names
 */
InputValidator.VALID_PROVIDERS = ["dashscope", "xai", "openai", "deepseek", "gemini"];
/**
 * ENHANCED: Security limits to prevent DoS and resource exhaustion
 */
InputValidator.MAX_TEXT_LENGTH = 10000; // Reduced from 50KB to 10KB for better performance
InputValidator.MAX_KEY_LENGTH = 500; // Reduced from 1000 to 500 characters
InputValidator.MAX_PATH_LENGTH = 1000; // Maximum file path length
InputValidator.MAX_CONFIG_DEPTH = 10; // Maximum object nesting depth
/**
