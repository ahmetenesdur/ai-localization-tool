...
const consoleLock = {
	queue: [] as Array<() => void>,
	isLocked: false,
	MAX_QUEUE_SIZE: 1000, // Maximum queue size to prevent unbounded growth

	async log(message: string): Promise<void> {
		return new Promise((resolve, reject) => {
			// Check queue size limit
			if (this.queue.length >= this.MAX_QUEUE_SIZE) {
				console.warn("Console lock queue is full, dropping message:", message);
				reject(new Error("Console lock queue is full"));
				return;
			}
		...
		if (this.queue.length > 0 && !this.isLocked) {
			...
			if (nextLog) {
				try {
					...
					console.error("Error in console lock queue processing:", error);
					this.isLocked = false;
					this._processQueue(); // Continue processing remaining queue items
				}
			}
		}
	},
...
export async function processLanguage(
	...
	try {
		...
		// Handle deleted keys if sync is enabled and removeDeletedKeys is true
		let deletedCount = 0;
		if (
			options.syncOptions?.enabled &&
			options.syncOptions?.removeDeletedKeys &&
			comparison.deletedKeys.length > 0
		) {
			console.log(
				`DEBUG: Found ${comparison.deletedKeys.length} deleted keys:`,
				comparison.deletedKeys
			);
			for (const deletedKey of comparison.deletedKeys) {
				if (deletedKey in flattenedTarget) {
					delete flattenedTarget[deletedKey];
					deletedCount++;
					console.log(`DEBUG: Removed deleted key: ${deletedKey}`);
				}
			}
			if (deletedCount > 0) {
				await consoleLock.log(
					`🗑️  Removed ${deletedCount} deleted keys from ${targetLang}`
				);
			}
		}

		console.log(
			`DEBUG: missingKeys.length=${missingKeys.length}, deletedCount=${deletedCount}`
		);

		// If no missing keys and no deleted keys were removed, skip processing
		if (missingKeys.length === 0 && deletedCount === 0) {
		...
		// If we only have deleted keys but no missing keys, save the updated target file
		if (missingKeys.length === 0 && deletedCount > 0) {
			console.log(`DEBUG: Saving target file with removed keys`);
			// Save to file
			const unflattened = ObjectTransformer.unflatten(flattenedTarget);
			await FileManager.writeJSON(targetPath, unflattened);

			await consoleLock.log(
				`💾 Target file updated (removed ${deletedCount} keys): ${targetLang}.json`
			);
			globalStats.languages[targetLang].timeMs = Date.now() - langStartTime;
			return {
				status: { completed: 0, total: 0, language: targetLang },
				savedMessage: `💾 Target file updated (removed ${deletedCount} keys): ${targetLang}.json`,
			};
		}
