...
import type { TranslationOptions, ApiConfig, ProviderRetryOptions } from "@/types";
...
export class DeepSeekProvider extends BaseProvider {
	...
	async translate(
		...
		// Safely extract retry options
		const retryOptions = options.retryOptions || {};
		const maxRetries = retryOptions.maxRetries ?? 3;
		const initialDelay = retryOptions.initialDelay ?? 2000;
	...
	async analyze(
		prompt: string,
		options: Partial<ApiConfig> & { maxRetries?: number; initialDelay?: number } = {}
	): Promise<string> {
