...
class FallbackProvider {
	...
	private _calculatePriority(text: string): number {
		if (!text) return 1;
	...
	async translate(
		...
		const availableProviders = this.providers.filter((_, index) => {
			const provider = this.providers[index];
			return provider && !this._isProviderDisabled(provider);
		});
		...
		while (currentAttempt < maxAttempts) {
			...
			// Check if providerData is valid
			if (!providerData) {
				currentAttempt++;
				this.currentIndex++;
				continue;
	...
	async analyze(prompt: string, options: TranslationOptions = {}): Promise<string> {
		const errors: ErrorRecord[] = [];
		const startTime = Date.now();
	...
	getStats(): Record<string, ProviderStats> {
		const stats: Record<string, ProviderStats> = Object.fromEntries(this.providerStats);
		...
		Object.keys(stats).forEach((provider) => {
			...
			if (providerStats) {
				...
				if (isDisabled && providerStats.disabledUntil) {
					providerStats.enablesInMs = Math.max(
						0,
						providerStats.disabledUntil - Date.now()
					);
				}
			}
		});
	...
	private _checkAndReRankProviders(): void {
		...
		const providerRanks = this.providers.map((provider, index) => {
			...
			if (total > 0) {
				...
				if (providerStats && stats.consecutiveFailures > 0) {
	...
	private _disableProvider(provider: ProviderEntry, timeoutMs: number = 2 * 60 * 1000): void {
	...
	private _isProviderDisabled(provider: ProviderEntry): boolean {
	...
	private _resetDisabledProviders(): void {
	...
	private _getProviderName(provider: ProviderEntry): string {
