class QualityChecker {
	...
	validateAndFix(sourceText, translatedText) {
		...
		if (this.rules.placeholderConsistency) {
			const { text, foundIssues, appliedFixes } = this.fixPlaceholders(sourceText, fixedText);
			fixedText = text;
		...
		if (this.rules.htmlTagsConsistency) {
			const { text, foundIssues, appliedFixes } = this.fixHtmlTags(sourceText, fixedText);
			fixedText = text;
			issues.push(...foundIssues);
			fixes.push(...appliedFixes);
		}
		...
		if (this.rules.punctuationCheck) {
			const { text, foundIssues, appliedFixes } = this.fixPunctuation(sourceText, fixedText);
			fixedText = text;
			issues.push(...foundIssues);
			fixes.push(...appliedFixes);
		}
	...
	checkLength(source, translated) {
		const sourceLengthWithoutSpaces = source.replace(/\s/g, '').length;
		const translatedLengthWithoutSpaces = translated.replace(/\s/g, '').length;
		const ratio = translatedLengthWithoutSpaces / sourceLengthWithoutSpaces;

		if (ratio < 0.5 || ratio > 2) {
			return [{
				type: 'length',
				message: 'Translation length is disproportionate',
				ratio
			}];
		}
	...
	fixPlaceholders(source, translated) {
		...
		sourcePlaceholders.forEach((placeholder) => {
			if (!fixedText.includes(placeholder)) {
				...
				const possiblePosition = this.findBestPlaceholderPosition(fixedText, source, placeholder);
				if (possiblePosition !== -1) {
					fixedText = fixedText.slice(0, possiblePosition) + 
							   placeholder + 
		...
		sourceTags.forEach((tag) => {
			if (!fixedText.includes(tag)) {
				...
				if (isClosingTag) {
					...
					const position = fixedText.indexOf(openingTag) + openingTag.length;
					fixedText = fixedText.slice(0, position) + tag + fixedText.slice(position);
				} else {
